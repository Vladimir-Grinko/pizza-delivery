    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div
              className="relative w-full max-w-[1100px] bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl overflow-hidden pointer-events-auto flex flex-col lg:flex-row"
              style={{ maxHeight: 'min(92dvh, 800px)' }}
            >
              {/* Close button */}
              <button
                onClick={() => onOpenChange(false)}
                className="absolute top-3 right-3 z-20 w-9 h-9 bg-white/90 dark:bg-zinc-800/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white dark:hover:bg-zinc-800 transition-colors shadow-lg"
              >
                <X className="w-4 h-4" />
              </button>

              {/* Left Column - Image */}
              <div className="relative h-[220px] sm:h-[280px] lg:h-auto lg:w-1/2 shrink-0 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-zinc-800 dark:to-zinc-900">
                {/* Skeleton */}
                {!imageLoaded && !imageError && (
                  <div className="absolute inset-0 bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 dark:from-zinc-700 dark:via-zinc-600 dark:to-zinc-700 animate-pulse" />
                )}

                {/* Error state */}
                {imageError && (
                  <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 dark:from-zinc-800 dark:to-zinc-900">
                ...
                <div className="absolute top-3 right-12 flex flex-col gap-2 items-end">
                  {product.size && (
              ...
              <div className="flex flex-col lg:w-1/2 overflow-y-auto">
                <div className="flex flex-col p-5 lg:p-6">
                  ...
                  {/* Add to Cart Button */}
                  <div className="mt-auto pt-4 border-t dark:border-zinc-700 pb-2">
                    <Button
                      onClick={handleAddToCart}
                      size="lg"
                      className="w-full h-13 sm:h-14 text-base sm:text-lg bg-primary hover:bg-primary/90 text-white shadow-lg hover:shadow-xl transition-all rounded-xl"
                    >
